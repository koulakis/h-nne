...
def test_hnne_deprecation_warning():
    with pytest.warns(
        DeprecationWarning, match="The argument `dim` is being deprecated"
    ):
        hnne = HNNE(dim=3)
...
def test_fit_transform_dim_deprecation():
    ...
    with pytest.warns(
        DeprecationWarning,
        match="The argument `dim` is being deprecated in favor of `override_dim`",
    ):
