...
def _grid_pairs_2d_bigaware(
    P: np.ndarray, r: np.ndarray, cell_size: float,
    ...
    def group_indices(gidx: int) -> np.ndarray:
        start = grp_starts[gidx]; cnt = grp_counts[gidx]
    ...
    for g in range(len(uniq_keys)):
        ...
        if cnt >= 2:
            ...
            I_list.append(idx[iu]); J_list.append(idx[ju])
    ...
    if len(uniq_keys):
        for ox, oy in OFFS:
            ...
            for g_src, g_dst in zip(src, dst):
                ia = group_indices(g_src); ib = group_indices(g_dst)
            ...
            for dx in range(-Rcells, Rcells + 1):
                for dy in range(-Rcells, Rcells + 1):
                    ...
                    if not np.any(ok):
                        continue
                    g_dst = pos[ok][0]
                    ia = group_indices(gpos); ib = group_indices(g_dst)
                    ia_big = ia[is_big[ia]]
                    if ia_big.size == 0:
                        continue
    ...
    I = np.concatenate(I_list); J = np.concatenate(J_list)
...
def _resolve_overlaps_allpairs_cap_2d(
    ...
    *,
    ...
    for _ in range(max_sweeps):
        ...
        im = iu[mask]; jm = ju[mask]
        dvm = dvec[mask]; distm = dist[mask]; needm = need[mask]
        ...
        move = ur * (needm + eps)
        wi, wj = inv_m[im], inv_m[jm]; ws = wi + wj
        ti = wi / (ws + 1e-12); tj = wj / (ws + 1e-12)

        dPi =  (ti * move)[:, None] * u
        dPj = -(tj * move)[:, None] * u

        # accumulate node increments
        add_x = np.bincount(im, weights=dPi[:,0], minlength=n) + np.bincount(jm, weights=dPj[:,0], minlength=n)
        add_y = np.bincount(im, weights=dPi[:,1], minlength=n) + np.bincount(jm, weights=dPj[:,1], minlength=n)
...
def separate_overlaps_minmove_grid_safe_2d(
    ...
    def _apply_batch(I, J, ur_local):
        ...
        if not np.any(mask):
            return 0, 0.0
        I2 = I[mask]; J2 = J[mask]
        dvec = dvec[mask]; dist = dist[mask]; need = need[mask]
        ...
        # accumulate node increments
        add_x = np.bincount(I2, weights=dPi[:,0], minlength=n) + np.bincount(J2, weights=dPj[:,0], minlength=n)
        add_y = np.bincount(I2, weights=dPi[:,1], minlength=n) + np.bincount(J2, weights=dPj[:,1], minlength=n)
    ...
    if np.any(need_final > eps):
        im = iu[need_final > eps]; jm = ju[need_final > eps]
        dvec = P[im] - P[jm]
        dist = np.linalg.norm(dvec, axis=1)
        u = np.zeros_like(dvec)
        nz = dist >= 1e-12
        u[nz] = dvec[nz] / dist[nz][:, None]
        if np.any(~nz): u[~nz, 0] = 1.0
        wi, wj = inv_m[im], inv_m[jm]; ws = wi + wj
        ti, tj = wi/(ws+1e-12), wj/(ws+1e-12)
        step = (need_final[need_final > eps] + eps)
        dPi =  (ti * step)[:, None] * u
        dPj = -(tj * step)[:, None] * u
        np.add.at(P, im, dPi); np.add.at(P, jm, dPj)
        if recenter_each_sweep:
            P -= P.mean(axis=0, keepdims=True)
    ...
    if np.any(need > eps_final):
        P = _resolve_overlaps_allpairs_cap_2d(
            P, r, gap=gap_final, ur=0.7, node_cap_frac=0.85,
            eps=eps_final, max_sweeps=80, recenter=True, mass_mode=mass_mode
        )
...
def _auto_edge_targets(A: np.ndarray, r: np.ndarray, k: int):
    ...
    cap_mult = 1.50
    if med_ratio > 2.0: cap_mult = 1.35
    if med_ratio > 4.0 or p90_ratio > 6.0:  cap_mult = 1.25
    if med_ratio > 8.0 or p90_ratio > 12.0: cap_mult = 1.18

    edge_relax = 0.30 if med_ratio <= 2.0 else 0.18
    if med_ratio > 4.0 or p90_ratio > 6.0:  edge_relax = 0.10
    if med_ratio > 8.0 or p90_ratio > 12.0: edge_relax = 0.05
...
def _first_auto_scale(P: np.ndarray, r: np.ndarray, k: int, max_sweeps: int = 120, pair_gap: Optional = None) -> np.ndarray:
    if len(P) <= 1:
        return P
    ...
    P = separate_overlaps_minmove_grid_safe_2d(
        P, r, tol=1e-9, mass_mode="area", cell_size=None, pair_gap=pair_gap, max_sweeps=max_sweeps
    )
    return P


def _align_to_anchors_2d(P: np.ndarray, A: np.ndarray) -> np.ndarray:
    P0 = P - P.mean(axis=0); A0 = A - A.mean(axis=0)
    H = P0.T @ A0
...
def _normalize_top_level_span_2d(
    ...
    if mode == "allpairs":
        ...
        return separate_overlaps_minmove_grid_safe_2d(
            P, r, tol=1e-9,
            cell_size=None, pair_gap=gap, max_sweeps=80)        
    else:
        return P
...
def pack_circles_from_anchors_2d_spacefill_k(
    anchors: np.ndarray, radii: Sequence[float], k: int,
    tol: float = 1e-9, mass_mode: str = "area",
    outer_sweeps: int = 1, pair_gap: Optional[float] = None,
    ...
    if auto_scale:
        ...
        P = separate_overlaps_minmove_grid_safe_2d(A_sc, r, tol=tol, mass_mode=mass_mode,
                                               cell_size=grid_cell_size, pair_gap=pair_gap, max_sweeps=160)
    ...
    for _ in range(max(1, outer_sweeps)):
        ...
        P = separate_overlaps_minmove_grid_safe_2d(
            P, r, tol=tol, mass_mode=mass_mode,
            cell_size=grid_cell_size, pair_gap=pair_gap, max_sweeps=160
        )
...
def _pack_children_inside_parent_dense_2d(
    A_local: np.ndarray, r_local: np.ndarray,
    parent_center: np.ndarray, parent_radius: float,
    k_local: int, outer_sweeps: int, mass_mode: str
):
    m = len(r_local)
    if m == 0:
    ...
    P_local = pack_circles_from_anchors_2d_spacefill_k(
        A_local, r_local, k=min(k_local, max(1, m-1)),
        tol=1e-9, mass_mode=mass_mode, outer_sweeps=outer_sweeps, pair_gap=None, post_normalize=False,
    )
...
def separate_overlaps_minmove_grid_safe_nd(
    ...
    if d > 6:
        ...
        for _ in range(max_sweeps):
            ...
            if not np.any(mask):
                break
            im = iu[mask]; jm = ju[mask]
            dvecm = dvec[mask]; distm = dist[mask]; needm = need[mask]
    ...
    if cell_size is None:
        rp = r[r > 0]
        base = max(2.0 * (float(np.max(rp)) if rp.size else 1.0),
            ...
            for off in offsets:
                ...
                for j in bucket:
                    ...
                    if dist < 1e-12:
                        ...
                        u = dvec / dist
                    move = need + tol
                    wi, wj = inv_m[i], inv_m[j]; ws = wi + wj
                    ti, tj = wi/(ws+1e-12), wj/(ws+1e-12)
                    P[i] += ti * move * u
                    P[j] -= tj * move * u
...
def pack_balls_from_anchors_nd_spacefill_k(
    anchors: np.ndarray, radii: Sequence[float], k: int,
    tol: float=1e-9, mass_mode: str="powerD", outer_sweeps: int=1
) -> np.ndarray:
    """
    Light ND packer (used only when D>2): resolve overlaps; no heavy edge projections.
    """
    A = np.asarray(anchors, float); r = np.asarray(radii, float)
    P = separate_overlaps_minmove_grid_safe_nd(A, r, tol=tol, mass_mode=mass_mode, max_sweeps=240)
    for _ in range(max(1, outer_sweeps)):
        P = separate_overlaps_minmove_grid_safe_nd(P, r, tol=tol, mass_mode=mass_mode, max_sweeps=240)
    return P

def _fit_children_inside_parent_nd(
    P_child: np.ndarray, r_child: np.ndarray,
    parent_center: np.ndarray, parent_radius: float,
    ...
    s = (fill_frac * parent_radius / ext) if ext > 1e-12 else (fill_frac * parent_radius)
...
def _pack_children_inside_parent_dense_nd(
    ...
    """
    P_local = pack_balls_from_anchors_nd_spacefill_k(A_local, r_local, k=k_local,
                                                     tol=1e-9, mass_mode=mass_mode, outer_sweeps=outer_sweeps)
    P_fit, r_fit = _fit_children_inside_parent_nd(P_local, r_local, parent_center, parent_radius, fill_frac=0.98)
    P_rel = P_fit - parent_center
    P_rel = separate_overlaps_minmove_grid_safe_nd(P_rel, r_fit, tol=1e-9, mass_mode=mass_mode, max_sweeps=200, pair_gap=0.0)
...
def _inflate_radii_balloon(
    ...
    for _ in range(steps):
        ...
        wall_slack = (R - wall_gap) - rho - r          # <= how much each can grow w.r.t. boundary
        ...
        P = separate_overlaps_minmove_grid_safe_2d(
            P, r, tol=1e-9, mass_mode="area", cell_size=None, pair_gap=0.0, max_sweeps=160
        )
...
def pack_hierarchy_iterative_k_nd(
    ...
    if D == 2:
        ...
        P_top = pack_circles_from_anchors_2d_spacefill_k(
                anc_top, rad_top, k=k, tol=1e-9, mass_mode=mass_mode_2d,
                outer_sweeps=outer_sweeps, pair_gap=None, auto_scale=True, post_normalize=True,
            )        
    else:
        P_top = pack_balls_from_anchors_nd_spacefill_k(
                anc_top, rad_top, k=k, tol=1e-9, mass_mode=mass_mode_nd, outer_sweeps=outer_sweeps
            )
    ...
    for lev in range(L - 2, -1, -1):
        ...
        c2p = {}
        child = labels_child; parent = labels_parent
        for c in np.unique(child):
            ps = np.unique(parent[child == c])
            if len(ps) != 1:
                raise ValueError(f"Inconsistent hierarchy at levels {lev}->{lev+1}: child {int(c)} -> {ps.tolist()}")
            c2p[int(c)] = int(ps[0])
        ...
        for c_lab in lab_child:
            parent_to_children_idx.setdefault(c2p[int(c_lab)], []).append(idx_map[int(c_lab)])
            ...
            if len(child_idx) == 1:
                ...
                if D == 2:
                    P_dense, r_grown = _pack_children_inside_parent_dense_2d(
                        A_local, r_local, p_center, p_radius,
                        k_local=min(k, max(1, len(child_idx)-1)),
                        outer_sweeps=outer_sweeps, mass_mode=mass_mode_2d
                    )
                else:
                    P_dense, r_grown = _pack_children_inside_parent_dense_nd(
                        A_local, r_local, p_center, p_radius,
                        k_local=min(k, max(1, len(child_idx)-1)),
                        outer_sweeps=outer_sweeps, mass_mode=mass_mode_nd
                    )
