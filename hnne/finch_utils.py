...
def _estimate_ivfpq_bytes(
    n: int, d: int, nlist: int, m: int, nbits: int, id_bytes: int = 4
) -> int:
...
def _default_faiss_kwargs(
    n: int, metric: str, d: int = 1024, *, ram_gb: int | None = None
) -> dict:
    """
...
def faiss_top1(
    ...
    *,
    ...
    if method == "hnsw":
        ...
        if verbose:
            print(
                f"[faiss_top1] HNSW: M={hnsw_M}, efC={hnsw_efC}, efS={hnsw_efS}, n={n}, d={d}"
            )
        ...
        D, I = index.search(Xn, 2)  # noqa: E741
        ...
        if need_cosine:
            ...
            nn_dist = (dist2 / 2.0).astype(
                np.float32
            )  # cosine distance = 1 - dot = dist2/2
        else:
    ...
    if verbose:
        print(
            f"[faiss_top1] IVF-PQ: nlist={nlist}, nprobe={nprobe}, m={m}, nbits={nbits}, "
    ...
    nn_dist = (
        (1.0 - nn_score).astype(np.float32)
        if need_cosine
        else np.sqrt(nn_score, dtype=np.float32)
    )
    return nn_idx, nn_dist
