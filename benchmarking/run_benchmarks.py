...
from benchmarking.utils import time_function_call
from hnne_v2 import HNNE
    ...
    for dataset_name, loader in loaders.items():
        try:
            ...
            _, time_elapsed_finch = time_function_call(
                hnne.fit_only_clustering, data, verbose=verbose
            )
            ...
            scores = pd.DataFrame(
                {
                    ...
                    "proj_time": [time_elapsed_projection]
                    + (len(knn_values) - 1) * [""],
            ...
            if dim == 2:
                ...
                if projection.shape[0] > points_plot_limit:
                    idx = np.random.choice(
                        projection.shape[0], points_plot_limit, replace=False
                    )
