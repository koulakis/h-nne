...
import typer
import math
from benchmarking.utils import time_function_call, plot_proj_grid, build_scores_overview
from benchmarking.data import dataset_loaders, DatasetGroup, dataset_validation_knn_values
from benchmarking.evaluation import format_metric, dim_reduction_benchmark
...
def run_eval(
    ...
    for dataset_name, loader in loaders.items():
        try:
            ...
            scores = pd.DataFrame({
                ...
                'proj_time': [format_time(time_elapsed_projection)] + (len(knn_values) - 1) * [''],
                'finch_time': [format_time(time_elapsed_finch)] + (len(knn_values) - 1) * ['']
            },
            ...
            if plot_projection and n_components == 2:
                ...
                if proj.shape[0] > points_plot_limit:
                    idx = np.random.choice(proj.shape[0], points_plot_limit, replace=False)
                    proj = proj[idx]
                    targ = targ[idx]
                ...
                if save_experiment:
                    ...
                    sc = ax.scatter(proj[:, 0], proj[:, 1], s=1, c=targ, cmap='Spectral')
                ...
                ds_title = str(dataset_name) if 'dataset_name' in locals() else f'Dataset {i}'  # or whatever identifier you have
    ...
    if plot_projection and len(all_projections) > 0:
        ...
        plt_grid = plot_proj_grid(all_projections, figsize, save_experiment, out_plotgrid_path)    
        plt_grid.show()
