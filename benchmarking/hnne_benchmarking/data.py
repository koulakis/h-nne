...
def load_mnist(data_path):
    ...
    x_mnist = np.vstack(
        [np.array(image).flatten() for image in x_train_mnist + x_test_mnist]
    )
...
def load_fmnist(data_path):
    x_train_fmnist, y_train_fmnist = zip(
        *FashionMNIST(data_path, train=True, download=True)
    )
    ...
    x_fmnist = np.vstack(
        [np.array(image).flatten() for image in x_train_fmnist + x_test_fmnist]
    )
...
def load_imagenet_test(data_path):
    ...
    imagenet_y = mat73.loadmat(data_path / "imagenet_ILSVRC_2012/val/labels.mat")[
        "labels"
    ]
...
def load_cifar_10(data_path):
    x_train_cifar10, y_train_cifar10 = zip(
    ...
    x_cifar10 = np.vstack(
        [np.array(image).flatten() for image in x_train_cifar10 + x_test_cifar10]
    )
...
def load_imagenet_train(data_path):
    imagenet_x = mat73.loadmat(data_path / "imagenet_ILSVRC_2012/train/data.mat")[
        "data"
    ]
    imagenet_y = mat73.loadmat(data_path / "imagenet_ILSVRC_2012/train/labels.mat")[
        "labels"
    ]
...
def load_google_news(data_path):
    data = models.KeyedVectors.load_word2vec_format(
        data_path / "google-news/GoogleNews-vectors-negative300.bin", binary=True
    ).vectors
...
def get_dataset_loader(datasets):
    return {dataset_name: dataset for dataset_name, dataset in datasets.items()}
