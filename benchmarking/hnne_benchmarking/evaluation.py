...
def dim_reduction_benchmark(
    ...
    if 0.9 * data.shape[0] > sample_train_set:
        print(
            f"Will subsample train datasets from {0.9 * data.shape[0]} to {sample_train_set} "
            f"for {k_values}-nn evaluation."
        )
    ...
    with tqdm(total=len(k_values) * kf.n_splits) as pbar:
            ...
            for train_index, test_index in kf.split(k_embedding, labels):
                if 0.9 * data.shape[0] > sample_train_set:
                    train_index = np.random.choice(
                        train_index, size=sample_train_set, replace=False
                    )
        ...
        if compute_trustworthiness:
            if data.shape[0] < 20000:
                ...
                tw = trustworthiness(
                    data[data_index], k_embedding[data_index], n_neighbors=5
                )
        else:
            tw = None
