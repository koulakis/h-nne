...
def plot_layout_level_2d(
    ...
    for lab, vals in items:
        ...
        ax.add_patch(
            Circle((x, y), r, fill=False, linewidth=1.2, edgecolor="k", alpha=0.9)
        )
        if draw_labels:
...
def _strict_child_to_parent_map(
    partitions: np.ndarray, child_level: int, parent_level: int
) -> Dict[int, int]:
    """
...
def plot_v2_nested_2d(
    ...
    *,
    ...
    parent_subset: Optional[
        Sequence[int]
    ] = None,  # if we want to show only some parents
    ...
    if parent_subset is not None:
        ...
        parents_items = [
            (lab, vals) for (lab, vals) in parents_items if lab in parent_subset
        ]
        if not parents_items:
    ...
    if len(num_child) != 0:
        print_str = str(
            f"Start@Parent L: {len(parents_items)} clusters, Children@{', '.join(map(str,num_child))} ({time_elapsed})"
        )
    else:
        print_str = str(
            f"Start@Parent L with {len(parents_items)} clusters ({time_elapsed})"
        )
    ...
    *,
    ...
    if labels is None:
        ...
        for ax, xy, title in zip(axes, arrs, titles):
            ax.scatter(xy[:, 0], xy[:, 1], s=point_size, c=other_color, alpha=0.5)
            ax.set_title(title)
            if equal_aspect:
